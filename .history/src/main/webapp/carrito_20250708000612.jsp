...
<html lang="es">
    ...
    <body class="d-flex flex-column h-100">
        ...
        <!-- Script para sincronizar carrito del servidor con localStorage -->
        <script>
            // Sincronizar carrito del servidor con localStorage
            document.addEventListener('DOMContentLoaded', function() {
                <% if (!carrito.estaVacio()) { %>
                    // Cargar productos del servidor al localStorage
                    carritoLocal = {
                        items: [
                                ...
                                ItemCarrito item = carrito.getItems().get(i); %>
                            {
                                ...
                                nombre: '<%= item.getNombre().replace("'", "\\'") %>',
                                precio: <%= item.getPrecio() %>,
                                talla: '<%= item.getTalla() %>',
                                categoria: '<%= item.getCategoria() %>',
                                cantidad: <%= item.getCantidad() %>,
                                stock: <%= item.getStock() %>,
                                descripcion: '<%= item.getDescripcion() != null ? item.getDescripcion().replace("'", "\\'") : "" %>'
                            }<%= i < carrito.getItems().size() - 1 ? "," : "" %>
                            <% } %>
                        ],
                        subtotal: <%= carrito.getSubtotal() %>,
                        envio: <%= carrito.getEnvio() %>,
