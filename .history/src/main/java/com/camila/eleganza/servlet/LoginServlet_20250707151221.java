...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("admin@eleganzaboutique.com".equals(correo.trim()) && "admin123".equals(password)) {
                System.out.println("Acceso de administrador detectado");
                
                // Crear usuario administrador temporal para la sesión
                Usuario adminUsuario = new Usuario();
                adminUsuario.setIdUsuario(0); // ID especial para admin
                adminUsuario.setNombre("Administrador");
                adminUsuario.setCorreo("admin@eleganzaboutique.com");
                adminUsuario.setTelefono("000-000-0000");
                adminUsuario.setCalle("Administración");
                adminUsuario.setCiudad("Medellín");
                adminUsuario.setCodigoPostal("00000");
                adminUsuario.setPais("Colombia");
                adminUsuario.setPassword("admin123");
                
                // Crear sesión usando SessionManager
                HttpSession session = request.getSession();
                SessionManager.crearSesion(session, adminUsuario);
                
                // Marcar como administrador en la sesión
                session.setAttribute("isAdmin", true);
                
                System.out.println("Sesión de administrador creada exitosamente");
            ...
            if (usuario != null) {
                ...
                if (usuarioDAO.verificarPassword(password, usuario.getPassword())) {
                    ...
                    // Crear sesión usando SessionManager
                    HttpSession session = request.getSession();
                    SessionManager.crearSesion(session, usuario);
                    
                    // Marcar como usuario regular (no admin)
