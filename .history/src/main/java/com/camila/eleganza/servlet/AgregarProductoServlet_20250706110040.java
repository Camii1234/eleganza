    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (nombre == null || nombre.trim().isEmpty() ||
                ...
                response.sendRedirect("../pages/agregar-producto.jsp?error=campos_vacios");
                return;
            }
            ...
            try {
                ...
                response.sendRedirect("../pages/agregar-producto.jsp?error=formato_numerico");
                return;
            }
            
            // Validar valores
            if (precio <= 0 || stock < 0) {
                response.sendRedirect("../pages/agregar-producto.jsp?error=valores_invalidos");
                return;
            }
            ...
            if (imagenPart != null && imagenPart.getSize() > 0) {
                ...
                if (contentType == null || !contentType.startsWith("image/")) {
                    response.sendRedirect("../pages/agregar-producto.jsp?error=tipo_archivo_invalido");
                    return;
                }
                
                if (imagenPart.getSize() > 5 * 1024 * 1024) {
                    response.sendRedirect("../pages/agregar-producto.jsp?error=archivo_muy_grande");
                    return;
                }
